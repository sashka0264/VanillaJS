...
// 6 Предположим, что два поезда движутся между двумя точками вдоль одного 
// отрезка на пути друг к другу. Напишите функцию, которая будет 
// считать через какое время два поезда встретятся.

/*const result = (x, y, distance) => {
    console.log(`Поезда встретятся через ${ (distance/(x+y)).toFixed(1) } час.`);
};
result(5, 125, 500);*/
